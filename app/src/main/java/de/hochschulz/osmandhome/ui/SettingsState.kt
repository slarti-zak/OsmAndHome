...
data class DiscoveryUiState(
    ...
    val filtered: List<HaState> get() = allStates.filter { it.attributes.hasLocation }.let { list ->
        if (query.isBlank()) list
        else list.filter {
            it.attributes.friendlyName?.contains(query, true) == true || it.entityId.contains(query, true)
        }
    }
}
...
class AppViewModel(app: Application) : AndroidViewModel(app) {
    ...
    fun setQuery(q: String) { _discovery.update { it.copy(query = q) } }

    fun loadEntities() {
        val url   = AppPrefs.getServerUrl(ctx)
        val token = "Bearer ${AppPrefs.getToken(ctx)}"
        val trust = AppPrefs.getTrustSsl(ctx)
        if (url.isBlank()) { _discovery.update { it.copy(error = "Server URL not configured") }; return }
        ...
        viewModelScope.launch {
            try {
                ...
                _discovery.update { it.copy(loading = false, allStates = states, configs = configs) }
            } catch (e: Exception) {
    ...
    private fun refreshConfigs() {
        _discovery.update { it.copy(configs = TrackedEntityRepository.getAll(ctx).associateBy { c -> c.entityId }) }
    }

    fun setEntityEnabled(entityId: String, enabled: Boolean) {
        TrackedEntityRepository.setEnabled(ctx, entityId, enabled); refreshConfigs()
    }

    fun upsertEntityConfig(cfg: TrackedEntityConfig) {
        TrackedEntityRepository.upsert(ctx, cfg); refreshConfigs()
    }

    fun enableAll() {
        TrackedEntityRepository.enableAll(ctx, _discovery.value.allStates
            .filter { it.attributes.hasLocation }.map { it.entityId })
        refreshConfigs()
    }

    fun disableAll()      { TrackedEntityRepository.disableAll(ctx); refreshConfigs() }

    fun resetToDefaults() {
        TrackedEntityRepository.resetToDefaults(ctx, _discovery.value.allStates
            .filter { it.attributes.hasLocation }.map { it.entityId })
        refreshConfigs()
    }
}